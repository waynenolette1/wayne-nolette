...
description: 'How OpsieBot uses Claude AI to parse Slack messages into atomic Terraform changes — and why validation before mutation prevents entire categories of bugs.'
pubDate: 2025-07-18
...
Claude AI handles all of these naturally. The prompt is simple: extract the old team name and new team name from the user's message. The cost per parse is a fraction of a cent (single short prompt), and the flexibility dramatically improves user experience.
...
## The PR as a Governance Gate

OpsieBot doesn't apply Terraform changes directly. It creates a PR. The PR goes through the standard review and merge process — the same governance gate as any other infrastructure change.
