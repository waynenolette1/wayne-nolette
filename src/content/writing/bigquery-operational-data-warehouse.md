...
**TL;DR**: IE Hub needed to join data from 6 different systems — PagerDuty, Jira, DataDog, Slack, Zoom, and organizational identity. I chose BigQuery as the operational data warehouse: 21 tables across 6 datasets, synced via scheduled Cloud Run services. The key advantages: columnar storage handles analytical queries efficiently, the managed service eliminates infrastructure overhead, and the SQL interface makes the data accessible to the AI agent's governed tools.
...
| `org_governance`   | 5      | Daily          | Employee records, service mappings, monitor governance, risk scores, SLA tracking |
