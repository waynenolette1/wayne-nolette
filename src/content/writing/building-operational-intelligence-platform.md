---
title: 'Building an Operational Intelligence Platform from Scratch'
...
**TL;DR**: Infrastructure Engineering had operational data scattered across DataDog, PagerDuty, Jira, Slack, Zoom, and Backstage. I built IE Hub — a unified platform that consolidates all of it into a single team-scoped view. 15 pages, 21 BigQuery tables, 16+ Cloud Run services. The key insight: cross-system correlation is where the real value lives, not individual dashboards.
...
Nobody had a unified view. Engineering managers spent hours building manual reports. Ownership was tribal knowledge. Governance was reactive — we caught problems after they caused impact, not before.

## Architecture Decision: Unified Platform vs. Point Solutions
...
1. **Cross-system correlation is the product.** Knowing a release violated change management AND caused an incident AND cost $X — that requires joined data. Separate dashboards can't do this.
...
3. **Maintenance scales linearly with separate tools** but stays constant with a unified platform. One BFF layer, one auth system, one deployment pipeline.
...
org_governance/       — employee records, service mappings, monitor governance
```
...
**DataDog Monitor Governance** — 9,000+ monitors, most with unclear ownership. This page surfaces monitors without owners, monitors that haven't fired in 6 months (stale), and monitors with governance violations.
...
**Start with 5 pages, not 15.** Team Hub, Incident Cost Analytics, and Monitor Governance drove 80% of the value. The other 12 pages were important but could have been built iteratively based on user feedback instead of upfront.
